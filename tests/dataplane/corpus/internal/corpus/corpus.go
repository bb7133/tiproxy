...
func Build() Manifest {
	...
	executeWithTypes := mustExecuteRequest(42, []any{int64(-7), ^uint64(0), []byte("blob"), nil}, true)
	executeWithReusedTypes := mustExecuteRequest(42, []any{int64(-7), ^uint64(0), []byte("blob"), nil}, false)
	...
	cases := []Case{
		...
		makeCase("stmt-prepare-metadata-deprecate-eof", "COM_STMT_PREPARE omits metadata EOF packets when CLIENT_DEPRECATE_EOF is negotiated.", []string{"CMD-022", "PS-004", "RSP-006"}, refs("pkg/proxy/backend/cmd_processor_exec.go:forwardPrepareCmd", "pkg/proxy/backend/cmd_processor_test.go:TestPreparedStmts"), modernCapNames, state("command", "ready"), []recordSpec{{"client_to_proxy", 0, [][]byte{commandPayload(pnet.ComStmtPrepare, []byte("SELECT ?"))}, nil}, {"backend_to_proxy", 1, prepareResponse(9, 1, 1, true), nil}}, expect("forward", "ready", nil, 0, "statement ID 9 becomes pending", "metadata groups omit classic EOF packets", "the response flushes once after the last declared column")),
		makeCase("stmt-execute-cursor", "COM_STMT_EXECUTE opens a read-only cursor.", []string{"CMD-023", "PS-002", "PS-004", "RSP-002", "RSP-008"}, refs("pkg/proxy/backend/cmd_processor_test.go:TestPreparedStmts"), legacyCaps, state("command", "statement_7_prepared"), []recordSpec{{"client_to_proxy", 0, [][]byte{stmtCommand(pnet.ComStmtExecute, 7, []byte{1, 0, 0, 0, 1, 0, 0, 0})}, nil}, {"backend_to_proxy", 1, [][]byte{{1}, columnDefinition("one"), pnet.MakeEOFPacket(pnet.ServerStatusAutocommit | pnet.ServerStatusCursorExists)}, nil}}, expect("forward", "cursor_open", []string{"SERVER_STATUS_AUTOCOMMIT", "SERVER_STATUS_CURSOR_EXISTS"}, 0, "statement remains pending", "rows are deferred until fetch")),
		makeCase("stmt-execute-types-reuse", "Two COM_STMT_EXECUTE requests freeze Go's null bitmap, signed/unsigned types, and new-params-bound reuse bytes.", []string{"CMD-023", "PS-004"}, refs("pkg/proxy/net/mysql.go:MakeExecuteStmtRequest", "pkg/proxy/net/mysql_test.go:TestPrepareStmts"), legacyCaps, state("command", "statement_42_prepared_with_4_params"), []recordSpec{{"client_to_proxy", 0, [][]byte{executeWithTypes}, nil}, {"backend_to_proxy", 1, [][]byte{ok}, nil}, {"client_to_proxy", 0, [][]byte{executeWithReusedTypes}, nil}, {"backend_to_proxy", 1, [][]byte{ok}, nil}}, expect("forward", "ready", []string{"SERVER_STATUS_AUTOCOMMIT"}, 0, "first execute supplies four parameter types", "second execute reuses the retained types", "null and unsigned markers stay exact")),
		makeCase("stmt-fetch", "COM_STMT_FETCH returns cursor rows and closes the cursor.", []string{"CMD-028", "PS-002", "RSP-002", "RSP-008"}, refs("pkg/proxy/backend/cmd_processor_test.go:TestPreparedStmts"), legacyCaps, state("command", "cursor_open"), []recordSpec{{"client_to_proxy", 0, [][]byte{stmtCommand(pnet.ComStmtFetch, 7, []byte{1, 0, 0, 0})}, nil}, {"backend_to_proxy", 1, [][]byte{{1, '1'}, pnet.MakeEOFPacket(pnet.ServerStatusAutocommit | pnet.ServerStatusLastRowSend)}, nil}}, expect("forward", "statement_7_prepared", []string{"SERVER_STATUS_AUTOCOMMIT", "SERVER_STATUS_LAST_ROW_SENT"}, 0, "cursor-open status is cleared")),
		makeCase("stmt-lifecycle-independent", "Two statement IDs independently retain long-data and cursor guards across ERR, partial fetch, reset, final fetch, and close.", []string{"CMD-023", "CMD-024", "CMD-025", "CMD-026", "CMD-028", "PS-001", "PS-002", "PS-003", "PS-005", "PS-006", "RSP-008"}, refs("pkg/proxy/backend/cmd_processor.go:updatePrepStmtStatus", "pkg/proxy/backend/cmd_processor.go:finishedTxn", "pkg/proxy/backend/cmd_processor_test.go:TestPreparedStmts", "pkg/proxy/backend/cmd_processor_test.go:TestMixPrepAndTxnStatus"), legacyCaps, state("command", "statements_7_and_8_prepared"), []recordSpec{
			{"client_to_proxy", 0, [][]byte{stmtCommand(pnet.ComStmtSendLongData, 7, []byte{0, 0, 'a', 'b', 'c'})}, nil},
			{"client_to_proxy", 0, [][]byte{mustExecuteRequest(8, nil, true)}, nil},
			{"backend_to_proxy", 1, [][]byte{{1}, columnDefinition("one"), pnet.MakeEOFPacket(pnet.ServerStatusAutocommit | pnet.ServerStatusCursorExists)}, nil},
			{"client_to_proxy", 0, [][]byte{mustExecuteRequest(7, nil, true)}, nil},
			{"backend_to_proxy", 1, [][]byte{errPacket}, nil},
			{"client_to_proxy", 0, [][]byte{stmtCommand(pnet.ComStmtFetch, 8, []byte{1, 0, 0, 0})}, nil},
			{"backend_to_proxy", 1, [][]byte{{1, '1'}, pnet.MakeEOFPacket(pnet.ServerStatusAutocommit)}, nil},
			{"client_to_proxy", 0, [][]byte{stmtCommand(pnet.ComStmtReset, 7, nil)}, nil},
			{"backend_to_proxy", 1, [][]byte{ok}, nil},
			{"client_to_proxy", 0, [][]byte{stmtCommand(pnet.ComStmtFetch, 8, []byte{1, 0, 0, 0})}, nil},
			{"backend_to_proxy", 1, [][]byte{{1, '2'}, pnet.MakeEOFPacket(pnet.ServerStatusAutocommit | pnet.ServerStatusLastRowSend)}, nil},
			{"client_to_proxy", 0, [][]byte{stmtCommand(pnet.ComStmtClose, 8, nil)}, nil},
		}, expect("forward", "ready", []string{"SERVER_STATUS_AUTOCOMMIT", "SERVER_STATUS_LAST_ROW_SENT"}, 0, "statement 7 long data survives execute ERR", "statement 8 cursor survives a fetch without LAST_ROW_SENT", "reset clears only statement 7", "final fetch and close clear statement 8", "migration is blocked until both independent guards clear")),
...
func mustExecuteRequest(stmtID uint32, args []any, newParamBound bool) []byte {
	request, err := pnet.MakeExecuteStmtRequest(stmtID, args, newParamBound)
	if err != nil {
		panic(fmt.Sprintf("build COM_STMT_EXECUTE corpus request: %v", err))
	}
	return request
}
