...
// SwapRouter replaces the recorded namespace after a real router Close/new
// lifecycle. Existing connection objects are rebound separately by RehydrateConn.
func (m *RecordingNamespaceManager) SwapRouter(rt router.Router) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.rt = rt
	m.ns = namespace.NewNamespaceForTest("default", "", rt)
}
