...
MARKER = "CP-ROUTE-WORKER real producer issuance and retirement passed"
...
def compile_binary(root, env):
    ...
    if r.returncode:
        raise RuntimeError("worker candidate did not compile:\n" + r.stdout + r.stderr)
    ...
    if len(binaries) != 1:
        raise RuntimeError(f"unexpected worker artifacts: {binaries}")
    return binaries[0]

def observe(binary, env):
    return factor_live.observe(binary, env, "worker_real_metrics_retirement_at_issuance")
...
def baseline(root, env):
    ...
    if r.returncode or "1 passed; 0 failed" not in r.stdout or MARKER not in r.stdout:
        raise RuntimeError("worker runtime failed:\n" + r.stdout)
