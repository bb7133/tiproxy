...
def main():
    ...
    # capture_composed_retained repeats the Static check; anchor on the composed
    # capture_inputs(true, true) call so the mutation stays on capture_composed.
    add("static-empty-not-source-qualified", SOURCE, "self.capture_inputs(true, true)?;\n        candidate.metrics =\n            if candidate.backend.mode() == control_topology::BackendSourceMode::Static {", "self.capture_inputs(true, true)?;\n        candidate.metrics =\n            if candidate.backend.mode() == control_topology::BackendSourceMode::Dynamic {", "static", "COMPOSE_STATIC_ACTUAL_EMPTY_SOURCE")
